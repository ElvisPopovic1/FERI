...
int pickCar(std::vector<ProbabilityCell> &probs, std::vector<bool> &carsUsed, int currentCar, int currentNode)
{
    ...
    for(i=0, j=0; i<solData.problemData.cars_number; i++)
    {
        ...
        {
            tc = solData.tensor[i];
            pc = solData.pheromones[currentCar==-1?0:currentCar].nodes[currentNode].carsPhero[i];
...
int pickNode(std::vector<ProbabilityCell> &probs, std::vector<bool> &nodesVisited, int currentCar, int currentNode, int startNeighbor)
{
        ...
        i<startNeighbor+solData.parameters.nFavorits && i<solData.problemData.dimension; i++)
    {
        ...
        k = tc.index;
        pc = solData.pheromones[currentCar].nodes[currentNode].neighborsPhero[k];
...
void UpdatePheromones(Solution &solution)
{
    ...
    for(c=0; c<solData.problemData.cars_number; c++)
        for(j=0; j<solData.problemData.dimension; j++)
            for(i=0; i<solData.problemData.cars_number; i++)
                solData.pheromones[c].nodes[j].carsPhero[i].tau *= one_minus_rho;
    ...
    for(c=0; c<solData.problemData.cars_number; c++)
        for(j=0; j<solData.problemData.dimension; j++)
            for(i=0; i<solData.problemData.dimension; i++)
                solData.pheromones[c].nodes[j].neighborsPhero[i].tau *= one_minus_rho;
    ...
    for(i=0; i<solution.dimension; i++) //current nodes
    {
        ...
        solData.pheromones[c].nodes[j].carsPhero[c].tau += update;
        solData.pheromones[c].nodes[j].neighborsPhero[n].tau += update;
    }
}
...
void LimitPheromones()
{
    ...
    for(c=0; c<solData.problemData.cars_number; c++)
        ...
        {
            for(i=0; i<solData.problemData.cars_number; i++)
                if(solData.pheromones[c].nodes[j].carsPhero[i].tau > solData.carPheroMax)
                    solData.pheromones[c].nodes[j].carsPhero[i].tau = solData.carPheroMax;
             for(i=0; i<solData.problemData.dimension; i++)
                if(solData.pheromones[c].nodes[j].neighborsPhero[i].tau > solData.nodePheroMax)
                    solData.pheromones[c].nodes[j].neighborsPhero[i].tau = solData.nodePheroMax;
        }
}
