...
bool SetParameters(int argc, char *argv[], Parameters &params)
{
    ...
    for(i=1; i<argc; i++)
    {
        ...
        else if(s=="-nowrite" || s=="--noWritefile")
        {
        ...
        else if(i<argc-1)
        {
            ...
            else if(s=="-cmr" || s=="--carMinRatio")
            {
                ss >> dValue;
                if(dValue > 0.0)
                    params.carMinRatio = dValue;
            }
            else if(s=="-nmr" || s=="--nodeMinRatio")
            {
                ss >> dValue;
                if(dValue > 0.0)
                    params.nodeMinRatio = dValue;
            }
            else if(s=="-cme" || s=="--carMinExp")
            {
                ss >> iValue;
                if(iValue >= 0)
                    params.carMinRatioExp = iValue;
            }
            else if(s=="-nme" || s=="--nodeMinExp")
            {
                ss >> iValue;
                if(iValue >= 0)
                    params.nodeMinRatioExp = iValue;
            }
            else if(s=="-st" || s=="--stall")
            {
                ss >> iValue;
                if(iValue >= -1)
                    params.stall = iValue;
...
void SetDefaultParams(Parameters &params)
{
    ...
    params.kappa = KAPPA;
    params.carMinRatio = params.nodeMinRatio = MIN_RATIO;
    params.carMinRatioExp = params.nodeMinRatioExp = MIN_RATIO_EXP;
    params.stall = STALL;
}
void WriteHint(std::string programName)
{
