...
void AddKappa(Solution s)
{
    ...
    if(solData.kappa.size()>(size_t)solData.parameters.kappa)
...
Solution GetMinKappa()
{
    ...
    for(i=0; i<(int)solData.kappa.size(); i++)
...
void calculateCarHeuristics()
{
    ...
    max = 0.0;
    for(c=0; c<solData.problemData.cars_number; c++) //max edge weight in all matrices and all cars
        for(j=0; j<solData.problemData.dimension; j++)
            for(i=0; i<solData.problemData.dimension; i++)
            {
...
for(j=0; j<solData.problemData.dimension; j++) //node
    for(i=0; i<solData.problemData.dimension; i++) //next nodes (neighbors)
    {
        ...
        solData.pheromones[j].neighborsPhero[index].eta /= solData.problemData.cars_number;
    }
}
...
bool InitSolverTest(ProblemData &problem, Solution &solution)
{
    ...
    static double ass_car_heuristics_with_passengers[] = {40.8787,13.6262}; //new and old algo
    static double ass_car_heuristics_without_passengers[] = {52.0341,26.0171};
    static double ass_node_heuristics_with_passengers[] = {57.0269, 11.3829};
    static double ass_node_heuristics_without_passengers[] = {305.092, 85.6112};
    ...
    sum3 = sum4 = 0.0;
    for(pheroNodeIt=solData.pheromones.begin(); pheroNodeIt!=solData.pheromones.end(); ++pheroNodeIt)
    {
