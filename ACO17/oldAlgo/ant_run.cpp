...
bool AntRun(Solution &antSolution)
{
    ...
    for(node=0; node<solData.problemData.dimension; node++)
    {
        ...
        do 
        {
            ...
            else //can't pick node into favorites
...
void UpdatePheromones(Solution &solution)
{
    ...
    for(i=0; i<solution.dimension; i++) //current nodes
    {
        ...
        std::cout << "Update pheromone in node " << j <<  " for car " << c << ", node " << n << std::endl;
    }
}

void resetPheromones(Solution &solution)
{
    int i, j;
    for(j=0; j<solData.problemData.dimension; j++)
    {
        for(i=0; i<solData.problemData.cars_number; i++)
            solData.pheromones[j].carsPhero[i].tau = PHERO_MAX;
        for(i=0; i<solData.problemData.dimension; i++) 
            if(i!=j)
                solData.pheromones[j].neighborsPhero[i].tau = PHERO_MAX;
            else
                solData.pheromones[j].neighborsPhero[i].tau = 0.0;
    }
...
void CalculateMaxMin(double cost)
{
    ...
    /* min */
    solData.carPheroMin = solData.carPheroMax * solData.parameters.carMinRatio;
    solData.nodePheroMin = solData.nodePheroMax * solData.parameters.nodeMinRatio;
}
...
void LimitPheromones()
{
    ...
    for(j=0; j<solData.problemData.dimension; j++)
    {
        for(i=0; i<solData.problemData.cars_number; i++)
        {
            ...
            if(solData.pheromones[j].carsPhero[i].tau < solData.carPheroMin)
                solData.pheromones[j].carsPhero[i].tau = solData.carPheroMin;
        }
        for(i=0; i<solData.problemData.dimension; i++)
        {
            if(i!=j)
            {
                ...
                if(solData.pheromones[j].neighborsPhero[i].tau < solData.nodePheroMin)
                    solData.pheromones[j].neighborsPhero[i].tau = solData.nodePheroMin;
            }
        }
    }
}
