...
static const std::string strUsage[] = {"Usage: ", "(ACO17 ... -p problemFile | ACO17 problemFile) \
 ...
 [(-cmr | --carMinRatio) nKappa] [(-nmr | --nodeMinRatio) nodeMinRatio] \
 [(-cme | --carMinExp) carMinExp] [(-nme | --nodeMinExp) nodeMinExp] \
 [(-st | --stall) stall] [(-nowrite | --noWritefile) noWriteFile]"};
...
static const std::string parametersDisplay2[] = 
{"Car min ratio","node min ratio","car min ratio exp","node min ratio exp","(old algorithm - exp ignored)","(new algorithm - ratio ignored)"};
static const std::string parametersDisplay3[] = {"Deadlock if the values remain the same","Write result to file"};
