...
int main(int argc, char *argv[])
{
    ...
    /* load shared library */
        ...
        if(params.algorithmType == ALT_NEW)
            solutionLib = dlopen("ACO17_CarS/bin/release/createSolution_new.so", RTLD_LAZY);
        else
            solutionLib = dlopen("ACO17_CarS/
            bin/release/createSolution_old.so", RTLD_LAZY);
    #else
        if(params.algorithmType == ALT_NEW)
    ...
    /* final display for IRACE */
    DisplayFinalPrice(solution);
