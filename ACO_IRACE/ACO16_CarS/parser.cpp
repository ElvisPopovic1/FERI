...
bool setParameters(int argc, char** argv)
{
    ...
    parData.minRatioNodes = MIN_RATIO;
    parData.minRatioCars = MIN_RATIO;
    pheroMin1 = pheroMax1 * parData.minRatioNodes;
    pheroMin2 = pheroMax2 * parData.minRatioCars;
    parData.seed = SEED;
    ...
    for(i=1; i<argc; i++)
    {
        ...
        if((strcmp(argv[i],"--pheroNodesRatio")==0 || strcmp(argv[i],"-pnr")==0) && i<argc-1)
        {
            p4 = atof(argv[i+1]);
            if(p4 > 0.0)
                parData.minRatioNodes = p4;           
        }
        if((strcmp(argv[i],"--pheroCarsRatio")==0 || strcmp(argv[i],"-pcr")==0) && i<argc-1)
        {
            p4 = atof(argv[i+1]);
            if(p4 > 0.0)
                parData.minRatioCars = p4;           
        }
    }
